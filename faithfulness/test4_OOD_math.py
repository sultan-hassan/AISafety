...
# 2. PROMPTS (Follow)
def get_follow_prompt(rule, query):
    return (f"<start_of_turn>user\n"
            f"Instruction: Follow the redefined arithmetic rule in the context strictly.\n"
            f"Context: {rule}\n"
            f"Question: {query}<end_of_turn>\n<start_of_turn>model\nAnswer:")


rule_target = "The symbol '+' means multiply (*)."
q_target = "What is 2 + 3?"
target_prompt = get_follow_prompt(rule_target, q_target)
...
print("\n2. Forcing '2 + 3 = 6'...")
# 3. STEERING (NEGATIVE)
# We use the vector from "Sky/Green" to try and fix "math".
coeffs = [0.0, -5.0, -8., -10.0, -15.0, -20.0]
